...
lb.id = ‘lb-overlay’;
lb.innerHTML = ` <div id="lb-bg"></div> <button id="lb-close" aria-label="Close">✕</button> <button id="lb-prev" aria-label="Previous">←</button> <button id="lb-next" aria-label="Next">→</button> <div id="lb-stage"><img id="lb-img" src="" alt="" draggable="false"></div> <div id="lb-counter"></div> <div id="lb-hint">scroll to zoom · esc to close</div>`;
...
`<div class="rw-masonry-item" data-lb-idx="${i}"> <img src="${img.path}" alt="${_esc(img.caption || '')}"> ${img.isMain ? '<span class="rw-masonry-main">★</span>' : ''} </div>`).join(’’);
galleryHTML = `<div class="reading-sec-label">${l === 'tr' ? 'görseller' : 'images'}</div> <div class="rw-masonry-grid" id="rw-thumbstrip">${thumbs}</div>`;
}
...
`<div class="rw-gallery-item rw-mat-thumb" data-mat-idx="${mi}" data-img-idx="${j}"> <img src="${img.path}" alt="${_esc(img.caption || '')}"> </div>`).join(’’);
...
matsHTML += lbl + `<button class="btn-3d" onclick="openViewer('${safePath}','${safeTitle}')"> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"> <path d="M12 2l9 4.5v9L12 20l-9-4.5v-9L12 2z"/><polyline points="12 2 12 20"/><polyline points="3 6.5 12 11 21 6.5"/> </svg>${l === 'tr' ? '3d görüntüle' : 'view 3d'}</button>`;
...
openReading(`${heroHTML}${infoHTML} <div class="reading-sec-label">${l === 'tr' ? 'açıklama' : 'about this work'}</div> <div class="reading-desc">${descParas ||`<p>${_esc(desc || ‘’)}</p>`}</div> ${galleryHTML}${matsHTML} `, badgeLabel, title);
...
openReading(`<div class="reading-title">${_esc(title)}</div> <div class="reading-meta">${_esc(pm.year || '')}</div> <div class="reading-sec-label">${lbl}</div> <div class="reading-text">${_esc(body)}</div>`, lbl, title);
}
...
openReading(`<div class="reading-title">${_esc(title)}</div> <div class="reading-meta">${_esc(ar.year || '')} · ${_esc(type || '')}</div> <div class="reading-sec-label">${_esc(type || '')}</div> <div class="reading-desc">${paras}</div>`, _esc(type || ‘’), title);
}
